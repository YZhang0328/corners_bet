...
Once that distribution is good enough, Kelly can compound capital even under realistic fees and stake caps, but full Kelly is too aggressive in practice. Fractional Kelly is more sensible because it gives a better risk-adjusted profile, and in this setting that matters more than maximising cash. Just as importantly, Kelly is highly sensitive to probability error, so stress testing is not optional.

\par\vspace{0.25cm}

Questions 2 and 3 are standard at the formula level: EV, distributions, and Kelly are well known. The real issue is whether the model probabilities behind those formulas are believable. That is why the assumptions have to be checked, especially with Monte Carlo. 
...
\noindent \textbf{Answer.} For stake sizing, I use Kelly rather than flat staking. The reason is that Kelly links bet size to estimated edge and current bankroll. If the estimated probabilities are correct, Kelly maximises expected log wealth. It is still aggressive, so I evaluate three fractions of Kelly instead of relying only on full Kelly.

For a bet with model win probability \(p\) and decimal odds \(o\), the full Kelly fraction is
\[
f^{*}=\frac{po-1}{o-1}.
\]
...
\begin{tabular}{lcccc}
\toprule
Strategy & Total PnL & Ending Bankroll & Turnover RoI & Max Drawdown \\
\midrule
100\% Kelly & \(+1{,}241{,}259.35\) & \(1{,}241{,}359.35\) & \(23.51\%\) & \(-88.58\%\) \\
50\% Kelly & \(+1{,}154{,}460.52\) & \(1{,}154{,}560.52\) & \(24.82\%\) & \(-44.33\%\) \\
25\% Kelly & \(+960{,}068.68\) & \(960{,}168.68\) & \(27.24\%\) & \(-22.21\%\) \\
\bottomrule
\end{tabular}
\end{center}

None of the three Kelly paths went bust in this realised sample, but full Kelly has extremely severe drawdowns. The most balanced line here is \(25\%\) Kelly: lower terminal PnL than full Kelly, but the highest turnover RoI and by far the mildest drawdown.
...
\noindent \textbf{Answer.} Not exactly. Kelly changes the stake size, but it does not repair probability miscalibration by itself. The edge still has to come from the probability model.
...
\begin{itemize}
    \item actual total PnL \(=+254.02\);
    \item Monte Carlo median PnL \(=+272.21\);
    \item Monte Carlo 5th to 95th percentile range \(=[+175.27,\; +377.45]\);
    \item actual percentile \(=38\)th.
\end{itemize}

So the realised outcome is below the model median, which means the model is still a bit optimistic, but it is not collapsing into the far left tail. Earlier versions of the pipeline were much worse on this point. After isotonic calibration and tail shrink, the assumed EV is not perfect, but it is much closer to a believable edge than the raw probabilities were.

That matters even more once Kelly is applied, because stake size is a direct function of estimated edge. This is the main reason I prefer showing \(25\%\) and \(50\%\) Kelly alongside full Kelly rather than trusting full Kelly blindly.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{../outputs/kelly_backtest/selection_edge_monte_carlo.png}
\caption{Monte Carlo distribution of unit-stake total PnL for the selected partition-B bets. This checks whether the model edge that feeds Kelly is believable.}
\end{figure}
